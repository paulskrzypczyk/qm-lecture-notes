...
4. In this question we will find the third excited state of the quantum harmonic oscillator. As in the notes, we will make a guess that the wavefunction is of the form $$ u_3(y) = v(y)e^{-\frac{1}{2}y^2}. $$
	1. Show that the function $$ v(y) = A_3y(y-c)(y+c), $$ where $A_3$ and $c$ are constants, is an odd function with 3 nodes (i.e.~is zero precisely 3 times). This shows that it is a good guess for the form of $v(y)$ for the third excited state.
