...
To the left of the step, which we denote by Region I, when $x < 0$, the potential vanishes, and the time-independent Schrödinger equation is identical as for the free particle, \eqref{e:TISE free particle}. We have already solved this equation, and hence we can write down the solution (as given in \eqref{e:sep spatial}), which is
\begin{equation}
