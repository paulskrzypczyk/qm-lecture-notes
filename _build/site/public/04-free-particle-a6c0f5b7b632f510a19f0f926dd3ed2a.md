...
```{math}
\Psi(x,t) &=& \left(\frac{1}{a(1+i\gamma t)\sqrt{2\pi}}\right)^{1/2 \\ 
& \times& \exp\left(\frac{-\left(x-\hbar k_0 t/M\right)^2}{4a^2(1+i\gamma t)}\right)\exp\left(ik_0(x-\hbar k_0t/2M)\right)
```
where 
```{math}
:label: e-gamma
