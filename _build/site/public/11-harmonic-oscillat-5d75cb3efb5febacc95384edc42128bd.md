...
\section{Approximation of an arbitrary potential well}
...
\section{The quantum harmonic oscillator potential}
...
\end{figure}
\section{Dimensionless co-ordinates}
...
\section{Behaviour far from the force centre}
...
\end{align}
Remember that we are interested in solutions \alert{for large values of $y$}. Therefore, just as we approximated the time-independent Schr\"odinger equation, we should also approximate the second derivatives of $e^{y^2}$ and $e^{-y^2}$. In particular,
\begin{align}
...
\end{equation}
\end{boxedeq}
\section{The first excited state}
...
\end{equation}
\end{boxedeq}
\section{The second excited state}
...
\section{General energy eigenstates and eigenvalues}
